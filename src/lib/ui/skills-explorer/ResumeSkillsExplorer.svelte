<script lang="ts">
	...
	let { skillRecords, skillCategories, chartReady = $bindable(false) }: Props = $props();
	...
	onMount(() => {
		...
		includedSkillIds = hydrateIncludedSkillIds(skillRecords);
		clientHydrated = true;
		return () => {
			...
			<section class="resume-skills-chart__toggle-group" aria-labelledby="toggle-{section.categoryId}">
				...
				<ul class="resume-skills-chart__toggle-list">
					{#each section.skills as skill (skill.id)}
						<li>
							<label class="resume-skills-chart__toggle" class:resume-skills-chart__toggle--off={!includedSkillIds.has(skill.id)}>
								<input
									type="checkbox"
