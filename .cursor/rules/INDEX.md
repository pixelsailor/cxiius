...
| Rule file                                  | One-liner                                                                                                                                                                                                 |
| :----------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`adr-compliance.mdc`](adr-compliance.mdc) | Active ADRs for build work; Planner distills `architecture-constraints.md`; Builder/Validator/Test consume distilled artifacts; follow `adr/GOVERNANCE.md`.                                                |

### Orchestration agents (manual)

Invoke by description or when running `.cursor/agents/` roles. Pair with [`.cursor/agents/`](../agents/) definitions.

| Rule file                                      | One-liner                                                                                                                                  |
| :--------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------- |
| [`orchestrator.mdc`](orchestrator.mdc)         | Controller for Plan → Build → Test → Validate loops under `.cursor/orchestrations/{task-id}/`; manifest, gates, remediation, human approval. |
| [`planner.mdc`](planner.mdc)                     | Produces `plan.md`, `acceptance-criteria.md`, and `architecture-constraints.md` when ADRs apply; no production code.                          |
| [`builder.mdc`](builder.mdc)                   | Implements `plan.md` within scope; SvelteKit/ADR patterns; writes `build-log.md` only among orchestration files.                             |
| [`test.mdc`](test.mdc)                         | Vitest / `vitest-browser-svelte` tests mapped to AC IDs; writes `test-report.md`.                                                            |
| [`validator.mdc`](validator.mdc)               | Fresh-context PASS/FAIL audit vs plan, ACs, and ADRs; writes `validation-report.md`; no fixes.                                             |
...
| Rule file                                                                | One-liner                                                                                                                                                                         |
| :----------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`svelte-data-loading-and-state.mdc`](svelte-data-loading-and-state.mdc) | Content routes: `async` `load()` awaits `$lib/content`, plain `data`, `prerender`; `{#await}` `:catch` when used; no-JS navigation; Svelte 5 runes — no legacy Svelte 4 patterns. |
...
| Rule file                                                    | One-liner                                                                                                                |
| :----------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------- |
| [`orchestration-artifacts.mdc`](orchestration-artifacts.mdc) | Who may edit `task-manifest.json`, `plan.md`, `architecture-constraints.md`, `build-log.md`, and other files under `.cursor/orchestrations/{task-id}/`. |
...
| **Manual**      | `alwaysApply: false` and **no** `globs` — not path-triggered. Attach by description when the task matches (orchestration roles, content interview). | [`orchestrator.mdc`](orchestrator.mdc), [`planner.mdc`](planner.mdc), [`builder.mdc`](builder.mdc), [`test.mdc`](test.mdc), [`validator.mdc`](validator.mdc), [`content-interview.mdc`](content-interview.mdc)                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
