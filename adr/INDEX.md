...
| ADR-002 | Architecture | [Data Fetching Patterns](ADR-002-data-fetching-patterns.md)                         | active | Content routes `await` `$lib/content` getters in `+page.ts`, return plain `data`, and use `prerender`; pages render from `data` without relying on `{#await}` for primary content (no-JS baseline)                           |
